...
<h5 > Results of searching for: <?php echo $search; ?></h5>
...
LEFT JOIN user ON post.author = user.user_id WHERE post.title LIKE '%{$search}%' || post.description LIKE '%{$search}%' || post.post_date LIKE '%{$search}%'
...
?>
                                ...
                                <div class="text-center">
                                    <?php
                                         ...
                                         $query2 = "SELECT post.post_id, post.title, post.description,post.post_img, post.post_date,post.category, category.category_name,user.username,post.author FROM post
                                         LEFT JOIN category ON post.category = category.category_id
                                    ...
                                    if (mysqli_num_rows($result2)) {
                                        ...
                                        if ($page_number > 1) {
                                            echo '<li class="page-item"><a class="page-link" href="search.php?search='.$search.'&page='.($page_number-1).'"><<</a></li>';                                        }
                                        ...
                                        for ($i = 1; $i <= $total_page; $i++) {
                                            ...
                                            echo '<li class= "page-item ' . $active . '"><a class="page-link" href="search.php?search='.$search.'&page='.$i.'">'.$i.'</a></li>';
